...
static PyObject *
cmsis_arm_hamming_window(PyObject *obj, PyObject *args){
  PyObject *pSrcA=NULL; // input
  float32_t *pSrcA_converted=NULL; // input
  PyObject *pSrcB=NULL; // input
  float32_t *pSrcB_converted=NULL; // input
  ...
  if ( PyArg_ParseTuple(args,"OO",&pSrcA) )
  {
    GETARGUMENT(pSrcA,NPY_DOUBLE,double,float32_t);
    blockSize = arraySizepSrcA ;
    pDst=PyMem_Malloc(sizeof(float32_t)*blockSize);
    ...
    //arm_sub_f32(pSrcA_converted,pSrcB_converted,pDst,blockSize);
    for(uint32_t i=0;i<arraySizepSrcA;i++){
    
    	pDst[i] = pSrcA_converted[i] *(0.54f - 0.46f* arm_cos_f32(2*PI*i/(arraySizepSrcA-1)));
    }
    FLOATARRAY1(pDstOBJ,blockSize,pDst);
    PyObject *pythonResult = Py_BuildValue("O",pDstOBJ);
...
static PyObject *
cmsis_arm_normalize(PyObject *obj, PyObject *args){
  PyObject *pSrcA=NULL; // input
  float32_t *pSrcA_converted=NULL; // input
  PyObject *pSrcB=NULL; // input
  float32_t *pSrcB_converted=NULL; // input
  ...
  if ( PyArg_ParseTuple(args,"O",&pSrcA) )
  {
    ...
    //arm_sub_f32(pSrcA_converted,pSrcB_converted,pDst,blockSize);
    dsp_normalize(pSrcA_converted,blockSize);
    for(uint32_t i=0;i<blockSize;i++){
      pDst[i] = pSrcA_converted[i];
    }
...
static PyObject *
cmsis_arm_stft(PyObject *obj, PyObject *args){
  PyObject *pSrcA=NULL; // input
  float32_t *pSrcA_converted=NULL; // input
  PyObject *pSrcB=NULL; // input
  float32_t *pSrcB_converted=NULL; // input
  ...
  if ( PyArg_ParseTuple(args,"O",&pSrcA))
  {
    GETARGUMENT(pSrcA,NPY_DOUBLE,double,float32_t);
    blockSize = arraySizepSrcA;
    pDst=PyMem_Malloc(sizeof(float32_t)*blockSize/2);
    ptmp=PyMem_Malloc(sizeof(float32_t)*blockSize/2);
    //printf("array size: %d\n",arraySizepSrcA);
    //arm_sub_f32(pSrcA_converted,pSrcB_converted,pDst,blockSize);
    printf("size = %d\n",arraySizepSrcA);
    dsp_stft(pSrcA_converted,ptmp,arraySizepSrcA);
    for(uint32_t i=0;i<(blockSize/2);i++){
      pDst[i] = ptmp[i];
    }

    FLOATARRAY1(pDstOBJ,blockSize/2,pDst);
    PyObject *pythonResult = Py_BuildValue("O",pDstOBJ);
...
//cmsis_dsp_reserve_freq_band
// 4 arguments
static PyObject *
cmsis_dsp_reserve_freq_band(PyObject *obj, PyObject *args){
  PyObject *pSrcA=NULL; // input
  float32_t *pSrcA_converted=NULL; // input
  //PyObject *pSrcB=NULL; // input
  //float32_t *pSrcB_converted=NULL; // input
  ...
  float32_t *ptmp = NULL;
  uint32_t blockSize; // input
  float32_t reserve_value=0.0f;
  uint32_t low_ind=0, high_ind=0;
  uint32_t tmp = 50;
  if ( PyArg_ParseTuple(args,"OIIf",&pSrcA,&low_ind,&high_ind,&reserve_value))
  {
    GETARGUMENT(pSrcA,NPY_DOUBLE,double,float32_t);
    ...
    pDst=PyMem_Malloc(sizeof(float32_t)*blockSize);
    printf("size : %d low : %d, high : %d  reserve: %f \n",blockSize,low_ind,high_ind,reserve_value);
    //printf("array size: %d\n",arraySizepSrcA);
    //arm_sub_f32(pSrcA_converted,pSrcB_converted,pDst,blockSize);
    //printf("size = %d\n",arraySizepSrcA);
    //dsp_stft(pSrcA_converted,ptmp,arraySizepSrcA);
    dsp_reserve_freq_band((float32_t*)pSrcA_converted,(float32_t*)ptmp,(uint32_t)blockSize,(uint32_t)low_ind,(uint32_t)high_ind,(float)reserve_value,(uint8_t)1);
    
    for(uint32_t i=0;i<(blockSize);i++){
      pDst[i] = pSrcA_converted[i];
      if(i<low_ind){
         pDst[i] = reserve_value;
      }else if(i>high_ind){
         pDst[i] = reserve_value;
      }
...
/*
void get_overlap_spectrum(float32_t nss[],				//output
                          float32_t data[],				//ssnoise
                          uint16_t window_size,			//the size of ssnoise
                          uint8_t overlap_weight[],		//overlap lwn
                          uint16_t overlap_size) {
*/
static PyObject *
cmsis_get_overlap_spectrum(PyObject *obj, PyObject *args){
  PyObject *pSrcA=NULL; // input
  float32_t *pSrcA_converted=NULL; // input
  //PyObject *pSrcB=NULL; // input
  //float32_t *pSrcB_converted=NULL; // input
  ...
  if ( PyArg_ParseTuple(args,"O",&pSrcA))
  {
    ...
    //dsp_stft(pSrcA_converted,ptmp,arraySizepSrcA);
    //dsp_reserve_freq_band((float32_t*)pSrcA_converted,(float32_t*)ptmp,(uint32_t)blockSize,(uint32_t)low_ind,(uint32_t)high_ind,(float)reserve_value,(uint8_t)1);
    get_overlap_spectrum(NULL,NULL,0,NULL,0);
    compensate_overlap_spectrum(NULL,NULL,NULL,NULL);
    for(uint32_t i=0;i<(blockSize);i++){
      pDst[i] = pSrcA_converted[i];
    }
...
*/
{"dsp_hamming_window" ,  cmsis_arm_hamming_window, METH_VARARGS,""},
{"dsp_normalize" ,  cmsis_arm_normalize, METH_VARARGS,""},
{"dsp_stft" ,  cmsis_arm_stft, METH_VARARGS,""},
{"dsp_reserve_freq_band" ,  cmsis_dsp_reserve_freq_band, METH_VARARGS,""},
#if 0
{"get_overlap_spectrum" ,  cmsis_get_overlap_spectrum, METH_VARARGS,""},
